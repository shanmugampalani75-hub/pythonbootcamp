...
def main():
    ...
    if not check_api_connection():
        ...
        st.info("Run: 'pyhton fastapi_mongo.py' to start the server")
        return
        ...
        with st.form("create_user_form"):
            ...
            if submitted:
                if name and email:
                    ...
                    if success:
                        st.success(f"User created successfulyy! ID: {result.get('user_id')}")
                        st.rerun()
                    else:
...
def dashboard_page():
    ...
    if user_success and posts_success:
        #metrics
        ...
        if users:
            ...
            with col2:
                ...
                if posts:
                    ...
                    posts_df['date'] = pd.to_datetime(posts-df['created_at']).dt.date
        ...
        if posts:
            ...
            for posts in recent_posts:
                st.write(f". **{post['title']}** - {pd.to_datetime(post['created_at']).strftime('%Y-%m-%d %H:%M:%S')}")
    else:
